...
const Board = ({ boardSize, mineNum, backToHome }) => {
    ...
    const freshBoard = () => {
        ...
        setBoard(prev => { return createBoard(boardSize, mineNum).board })
        setMineLocations(prev => { return createBoard(boardSize.mineLocations).mineLocations })
        let temp = 0;
        return (
            <>
                {board.map((x) => { return <div id={temp++} style={{ display: 'flex' }} ></div> })}
            </>
        )
    }
